...
The other thing to note is the heteroscedaciticy you can see among the variables.

Let's try a standard Normal regression model with pooled data, and another with an interaction term

```{r data5}
hiking_pooled <- stan_glm(mph ~ ele_gain_ft + ele_loss_ft + HR_avg,
...
hiking_pooled_int <- stan_glm(mph ~ ele_gain_ft + ele_loss_ft + HR_avg + ele_gain_ft:HR_avg,
                        data = df, family = gaussian,
                        prior_intercept = normal(2,1),
                        prior = normal(0, 0.5, autoscale = T),
                        prior_aux = exponential(1, autoscale = T),
...
mcmc_trace(hiking_pooled_int, size=0.1)

print("rhat")
rhat(hiking_pooled)
rhat(hiking_pooled_int)
print("neff ratio")
neff_ratio(hiking_pooled)
neff_ratio(hiking_pooled_int)

tidy(hiking_pooled, effects=c("fixed", "aux"),
     conf.int=T, conf.level=0.99)
tidy(hiking_pooled_int, effects=c("fixed", "aux"),
     conf.int=T, conf.level=0.99)
